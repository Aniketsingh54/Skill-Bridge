...
def test_analyze_pdf_resume_path_uses_pdf_ingestion(monkeypatch):
    class StubPDFResumeIngestor:
    ...
    monkeypatch.setattr("app.main.PDFResumeIngestor", StubPDFResumeIngestor)
    monkeypatch.setattr("app.main.AIGenerationStrategy", StubAIStrategy)
    ...
    assert response.status_code == 200
    payload = response.json()
    assert payload["ingestor_used"] == "StubPDFResumeIngestor"
